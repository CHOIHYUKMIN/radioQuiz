...
const App = () => {
  ...
  const [isMicTestMode, setIsMicTestMode] = useState(false);
  ...
  useEffect(() => {
    ...
    const shouldRunSTT = (isPlayingStore || isMicTestMode) && isWhisperReadyState && isServiceRunning;
        ...
        startSTT((text: string, isNewChunk: boolean) => {
          ...
          if (cleanText) {
            setSubtitles((prev) => {
              ...
              if (isNewChunk) {
                return [cleanText, ...prev.slice(0, 50)];
              } else {
                return [cleanText, ...prev.slice(1)].slice(0, 50);
              }
            });
          ...
          console.log('App: 1초 대기 후 STT 안전 재가동 예약...');
          setTimeout(() => {
            if (sttStopRef.current) sttStopRef.current = null;
            setSttRestartTrigger(prev => prev + 1);
          }, 1000);
        }).then(stop => {
          console.log('App: STT engine started successfully');
  ...
  }, [isPlayingStore, isMicTestMode, isWhisperReadyState, sttRestartTrigger]);
  ...
  const handlePlayChannel = async (channel: Channel) => {
    setSubtitles((prev) => [`🔊 ${channel.name} 📡 연결 중...`, ...prev.slice(0, 50)]);
        ...
        <View style={styles.rightColumn}>
          <View style={styles.subtitleHeader}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <TouchableOpacity
                onPress={async () => {
                  console.log('App: Mic Test mode toggled', !isMicTestMode);
                  if (!isMicTestMode) {
                    if (Platform.OS === 'android') {
                      const granted = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.RECORD_AUDIO);
                      if (!granted) {
                        Alert.alert('알림', '마이크 권한이 없습니다! 휴대폰 설정 > 애플리케이션 > 권한 에서 마이크를 허용해주세요.');
                        return;
                      }
                    }
                    ...
                    isStartingSTTRef.current = false;
                    setTimeout(() => {
                      setIsMicTestMode(true);
                      setSubtitles(prev => [`🎙️ 마이크 테스트 시작! (말씀해 보세요)`, ...prev.slice(0, 50)]);
                    }, 500);
                  } else {
                    setIsMicTestMode(false);
                    if (sttStopRef.current) {
                      sttStopRef.current();
                      sttStopRef.current = null;
                    }
                    isStartingSTTRef.current = false;
                    setSubtitles(prev => [`🛑 마이크 테스트 종료`, ...prev.slice(0, 50)]);
                  }
                }}
                style={[styles.debugBtn, { backgroundColor: isMicTestMode ? '#E63946' : '#28A745' }]}
              >
                <Text style={styles.debugBtnText}>{isMicTestMode ? '마이크 끄기' : '마이크 켜기'}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => {
                  console.log('App: DEBUG button clicked');
                  setSubtitles(prev => [`TEST: ${new Date().toLocaleTimeString()}`, ...prev.slice(0, 50)]);
                }}
                style={styles.debugBtn}
              >
                <Text style={styles.debugBtnText}>TEST</Text>
              </TouchableOpacity>
              {quizDetected && (
...
const styles = StyleSheet.create({
  ...
  debugBtn: { backgroundColor: '#333', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 8, marginLeft: 10 },
  debugBtnText: { color: '#FFF', fontSize: 14, fontWeight: 'bold' },
